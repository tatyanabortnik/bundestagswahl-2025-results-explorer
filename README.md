...
1. Uniqueness - the keys should be unique per my findings
2. URL search params compatability — the keys can be stored directly in the URL.
3. Simple data lookup — I decided to store data in a Map using these keys to provide fast lookups for the autocomplete.
...
Data flow: `kerg2.csv` → `ElectionDataProvider` → components consume the full dataset via `useElectionData()`, or area-specific data (derived from URL search params via `BrowserRouter`) via `useAreaSelection()`. 
